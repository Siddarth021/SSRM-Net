...
from SSL_KD_Phase1_kaggle.evaluation.multilabel_metrics import compute_multilabel_metrics

from SSL_KD_Phase1_kaggle.datasets.cinc2020_dataset import CINC2020ECG
...
from SSL_KD_Phase1_kaggle.training.finetune_teacher import TeacherFineTuneModel
...
def run_verify():
    ...
    age_gender = torch.randn(8, 5).to(device)
    ...
    model.eval()
    with torch.no_grad():
        val_logits = model(lead2, morphology, age_gender)
...
def main():
    ...
    if os.path.exists(ssl_path):
        model.encoder.load_state_dict(torch.load(ssl_path), strict=False)
        print(f"Loaded pretrained SSL weights (with strict=False) from {ssl_path}", flush=True)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for batch in train_pbar:
            ...
            # Autocast disabled to prevent cuDNN crash
            logits = model(lead2, morphology, age_gender)
